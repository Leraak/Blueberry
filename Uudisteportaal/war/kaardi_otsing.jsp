...
<html>
  <head>
    ...
    <title>Mustikauudised - Kaardiotsing</title>
    <meta charset="UTF-8">
  ...
  Class.forName("com.mysql.jdbc.GoogleDriver");
	...
	ResultSet rs = conn.createStatement().executeQuery(
	    "SELECT uudiseid, uudise_pealkiri, uudise_sisu, pildi_url from uudised where uudiseid in (" + otsing + ")");
			...
			<a href="/bsuudis.jsp?id=<%= id %>">
		      ...
		      <a href="/bsuudis.jsp?id=<%= id %>">
		        ...
		        <%
				 ...
				 %>
					<!-- LEMMIKUUDIS -->
					<form action="/lemmikutest" method="post">
								<input type="hidden" name="lemmikuID" value="<%= id %>" /> 
					        	<button type="submit" class="btn btn-danger">Eemalda lemmikutest</button>
					        </form>
		        <%			rs2.first();
		        			break;
				    		}//if
						...
						if(lemmikUudis == false){
							...
							<form action="/lemmikutesse" method="post">
								<input type="hidden" name="lemmikuID" value="<%= id %>" /> 
					        	<button type="submit" class="btn btn-info">Lisa lemmikutesse</button>
					        </form>
					    <%
				    	rs2.first();
			    		}
